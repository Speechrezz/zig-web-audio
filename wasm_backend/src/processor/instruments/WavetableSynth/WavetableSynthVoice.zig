...
const WavetableOsc = dsp.WavetableOsc;

buffer: audio.AudioBuffer = .empty,
osc: WavetableOsc = .init,
...
pub fn init(self: *@This()) void {
    ...
    // TODO: REMOVE ME! Temporary wavetable initialization
    const table = self.osc.getTable();
    WavetableOsc.fillSawWave(table);
    self.osc.analyzeTable();
}
...
pub fn startNote(self: *@This(), note_frequency: f32, velocity: f32, _: i32) void {
    ...
    self.osc.reset();
    self.osc.bandLimitAtFrequency(note_frequency);
    self.adsr.noteOn();
