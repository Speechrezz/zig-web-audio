...
pub const VTable = struct {
    ...
    // --Serialization--
    toJsonSpec: *const fn (*anyopaque, write_stream: *std.json.Stringify) std.io.Writer.Error!void = toJsonSpecFallback,
    save: *const fn (
        ptr: *anyopaque,
        serialization_context: *const anyopaque,
        write_stream: *std.json.Stringify,
    ) std.io.Writer.Error!void = saveFallback,
    load: *const fn (
        ptr: *anyopaque,
        std.mem.Allocator,
    ...
    fn loadFallback(_: *anyopaque, _: std.mem.Allocator, _: *anyopaque, _: std.json.ObjectMap) !void {}
};
...
pub fn save(self: *@This(), serialization_context: *const anyopaque, write_stream: *std.json.Stringify) !void {
    ...
    try self.vtable.save(self.ptr, serialization_context, write_stream);
...
pub fn load(self: *@This(), allocator: std.mem.Allocator, serialization_context: *anyopaque, parsed: *const std.json.Value) !void {
    ...
    try self.vtable.load(self.ptr, allocator, serialization_context, object);
}
